...
def render(a):
    ...
    lines = [
        ...
        "## 9. FORMAL验收", "", table(["项目", "计划/要求", "实际结果"], [["冻结候选", manifest.get("frozen_candidate_id"), formal.get("candidate_id")], ["计划ID", manifest.get("formal_plan_id"), formal.get("plan_id")], ["执行路径", "阶段1已认证模拟路径；Server Flow调用数=0", formal.get("execution_path", "未记录")], ["独立种子集", "与CALIBRATION不同", formal.get("sample", {}).get("seed_set_hash")], ["样本数", formal.get("planned_sample_count"), formal.get("sample", {}).get("paid_entry_count")], ["有效尝试上限", manifest.get("formal_attempt_limit"), formal.get("attempt")], ["执行有效性", True, formal.get("execution_valid")], ["真实结论", "硬指标+综合分+FORMAL联合判定", formal.get("status")]]), "", "### 9.1 独立性证明", "", table(["检查项", "要求", "结果"], [["候选冻结", "FORMAL前参数hash固定", formal.get("candidate_hash", "未记录")], ["种子/trace", "与CALIBRATION独立", formal.get("sample", {}).get("seed_set_hash")], ["执行进程/输入", "独立且hash密封", formal.get("input_hashes", "未记录")], ["阶段1Server Flow认证", "绑定认证证据hash", formal.get("stage1_server_flow_certification_sha256", "未记录")], ["FORMAL Server Flow调用数", 0, formal.get("server_flow_call_count", "未记录")], ["独立性总判定", True, formal.get("independent_from_calibration")]]), "",
        "## 10. 豁免、不可达与阻塞", "", table(["指标", "状态", "原因", "批准"], [[x.get("metric_id"), x.get("status"), x.get("reason"), x.get("approval")] for x in waivers]), "", "## 11. 最终交付建议", "", "进入阶段5封存；封存后执行一次非阻塞Server Flow硬指标审计，审计结果只警告且不改变既有状态。" if formal.get("execution_valid") else "等待有效FORMAL或补充证据后再封存。", "",
