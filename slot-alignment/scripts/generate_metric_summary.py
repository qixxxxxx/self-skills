...
def render(library):
    ...
    lines += ["", "全部N/J/P/B活动正式项都只按“距离 <= C级通过值”判定，任一失败不得用分数补偿。原版证据不足的实例转为观察；分布缺桶时，证据达标档位仍正式评价，低样本档位和整体移动量转观察。存在观察项时最多只能给出有限范围通过。当前已落定N/J/P/B单项分、卡分和分类分；跨分类综合权重留待后续授权。", ""]
    return "\n".join(lines)
