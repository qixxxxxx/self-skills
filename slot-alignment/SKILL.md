---
name: slot-alignment
description: 根据 Slot 游戏原版采集协议、规则、规格、Runtime和用户认证的Python模拟脚本建立统一主流玩法画像，开工前完成指标库覆盖与业务决策预检，匹配版本化且语义去重的指标包，对齐RTP、盘面符号、中奖构成、玩法过程、奖励修饰与持久状态，执行结构可达性预检、自动可审计豁免、硬指标门禁、100分评分、受控CALIBRATION、独立FORMAL验收和中文交付，并维护全量中文指标汇总。用于老虎机原版数值/体验对齐、盘面重复或中奖单一诊断、指标规划、参数搜索、候选验收、不可达与豁免审查，或生成阶段1至阶段5固定结构产物及`阶段4-数值对齐报告.md`。
---
...
正式执行五阶段前先做只读、低成本预检，不运行CALIBRATION或FORMAL：解析资料、玩法画像、当前指标目录与统计能力，运行`compile_metric_instances.py`并输出`metric_extension_proposal.json`。玩法画像包含未知玩法、要求的指标包不存在，或存在指标包但没有有效Primary/Guard/阻塞Audit承接时，必须一次性向用户提交完整扩展提议；提议同时列出玩法语义目录、指标包、唯一Owner、测量合同、报告展示和脚本认证影响。用户决定后先完成指标库和脚本适配，再由用户只认证最终Python脚本hash。`preflight_decision_gate.status=通过`、指标库缺口数为0且扩展决策为“无需扩展/已完成”后，才视为正式开工。
...
开工前把路径、目标、作用域、指标库扩展、参数权限和最终脚本认证一次性决策完毕。正式执行加载`continuous_execution_policy.v1.json`后连续推进：临时错误同输入自动重试2次；报告/缓存失效自动重建；候选失败自动继续；FORMAL失败自动回CALIBRATION并尝试下一个冻结候选，最多3个；FORMAL样本不足先在密封预算阶梯内扩样，仍不足则自动豁免对应实例并重生成阶段3/4结果；结构不可达证据有效时停止扩张预算、自动豁免受影响实例并重生成评分。
