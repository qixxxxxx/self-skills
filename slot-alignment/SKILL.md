...
纯样本计数不足和有密封证据的结构不可达可按预授权政策自动豁免；定义、目标、输出、实现或配置错误仍然阻塞。存在任一必需豁免时，最终最多显示`豁免后通过`。
...
- FORMAL样本不足先在密封预算阶梯内扩样，仍不足时按精确实例自动豁免并重生成阶段3/4；
- 结构不可达证据有效时停止仅靠追加预算，自动处理证据列出的精确实例。

资料、权限、实现错误、玩法规则变更、预检遗漏的新指标扩展或持续外部错误不能自动处理时，保存命令、输入hash、错误和最小恢复动作，以`不通过`或`无法判定`完成最终报告。边界与责任见[91-边界停止与责任.md](references/91-边界停止与责任.md)。
