...
def audit(state: Any) -> list[str]:
    ...
    required = {
        ...
        "verification", "blockers", "final_evidence", "created_at", "updated_at",
    }
    ...
    errors.extend(text_list_errors(state["changed_files"], "changed_files"))
    ...
    if any(not Path(path).is_absolute() for path in state["changed_files"]):
        errors.append("changed_files 必须全部为绝对路径")
    ...
    step_required = {
        ...
        "started_at", "completed_at", "skip_reason",
    }
    safe_steps = []
...
def integrity_errors(path: Path, state: dict[str, Any]) -> list[str]:
    ...
    if state.get("schema_version") in {1, 2} and journal and isinstance(journal.get("state_after"), dict):
        snapshot = journal["state_after"]
        if snapshot.get("schema_version") == 3 and snapshot.get("task_id") == state.get("task_id"):
...
def save(path: Path, state: dict[str, Any], event: dict[str, Any]) -> None:
    ...
    if errors:
        fail("；".join(errors))
    write_json_atomic(journal_path(path), journal_document(path, state, event))
...
def add_records(state: dict[str, Any], args: argparse.Namespace) -> None:
    changed = absolute_paths(getattr(args, "changed", []), state["workspace_root"])
    state["changed_files"] = unique(state["changed_files"] + changed)
    stamp = now()
...
def cmd_migrate(args: argparse.Namespace, path: Path) -> None:
    with lock(path):
        if journal_path(path).exists():
            journal, error = try_read_dict(journal_path(path))
            if journal and isinstance(journal.get("state_after"), dict) and journal["state_after"].get("schema_version") == 3:
                fail("已存在 schema 3 journal；若 state 落后请运行 recover")
            if not args.force:
                fail(f"已有 journal 且不可用于迁移：{error or '格式不兼容'}；核对后可用 migrate --force")
        ...
        if version == 3:
            fail("状态已是 schema_version 3；journal 缺失时请运行 repair")
        if version not in {1, 2}:
            fail("只支持从 schema_version 1 或 2 迁移")
        ...
        state.setdefault("decisions", []).append({"at": now(), "text": f"状态协议从 schema {version} 迁移至 3"})
        save(path, state, {"type": "migrate", "summary": f"状态协议从 schema {version} 迁移至 3"})
    print(f"OK: migrate revision={state['revision']} schema=3")
...
def cmd_read(args: argparse.Namespace, path: Path) -> None:
    ...
    if args.command == "audit":
        print(f"OK: schema=3 revision={state['revision']} status={state['status']}")
        return
    ...
    print("已知变更文件: " + (" | ".join(state["changed_files"]) if state["changed_files"] else "无"))
    print("关键决定: " + (" | ".join(item["text"] for item in state["decisions"]) if state["decisions"] else "无"))
    ...
    with lock(path):
        ...
        if args.command == "revise":
            ...
            if args.goal is not None:
                state["goal"] = clean(args.goal, "目标")
            if args.scope is not None:
                state["scope"] = unique(args.scope)
            if args.constraint is not None:
                state["constraints"] = unique(args.constraint)
            reason = clean(args.reason, "修订原因")
            decision = f"任务修订：{reason}"
            ...
            event.update(summary=decision)
        elif args.command == "add-step":
            had_pending = next_pending(state) is not None
            raw = args.title + (" => " + args.acceptance if args.acceptance else "")
            step = parse_step(raw, len(state["steps"]) + 1)
            state["steps"].append(step)
            if state["status"] == "in_progress" and not had_pending and not state["current_step"]:
                state["next_action"] = f"开始 {step['id']}：{step['title']}"
            ...
            step["evidence"] = unique(step["evidence"] + args.evidence)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    checkpoint.add_argument("--changed", action="append", default=[])
    ...
    complete.add_argument("--changed", action="append", default=[])
