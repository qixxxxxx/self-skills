...
3. `completed` 有证据；`skipped` 同时有理由和现场证据。
4. 活动任务有唯一 `next_action`；活动阻塞只存在于 `blocked` 任务。
5. 完成任务所有步骤已终结、至少一个实际完成步骤、存在最终证据和非空验证、无活动阻塞。
6. journal 的 `state_path`、`task_id`、revision、时间和 `state_after` 与 state 一致。
...
migrate     把 schema 1 或 2 状态迁移为 schema 3
...
`migrate` 支持 schema 1 和 2。若 state 缺失，它会尝试从旧 `events.jsonl` 最后一条完整 `state_after` 快照读取；旧日志没有快照时不能自动恢复。旧版 `skipped` 步骤缺少证据时，逐项传入 `--skip-evidence 'S1=现场证据'`；完成任务没有验证记录时，传入非空 `--final-verification '最终验证结果'`。不得为了迁移编造证据。旧日志迁移后只保留作人工审计，不参与新版恢复。
