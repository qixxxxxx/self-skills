...
description: 通过项目内状态文件、原子检查点、有界近期历史、失败尝试、文件生命周期、恢复门禁和证据驱动的步骤转换，使长任务在上下文压缩、Goal 自动续跑、会话恢复、工具中断、状态查询或 Agent 交接后从唯一下一动作继续，避免重做已完成工作、重复失败路线或把中间汇报误当成停止指令。用于预计跨多轮或长时间、步骤较多、修改多个文件、反复试验、运行长命令，或用户要求持续执行、Plan/Goal 配合、断点续做、压缩后恢复及 Agent 自行维护进度文件的任务。
---
...
- 用户询问“进度、当前候选、预计耗时、为什么这样做、是否足够灵活、输出中间结果”等观察性问题，不代表暂停、取消、缩小范围或替换原任务。除非用户明确说“停止、暂停、先不要继续、取消”或给出与原目标冲突的新指令，否则任务状态保持`in_progress`。
- 收到观察性问题后先运行`resume`核对现场；需要形成新结论时写`checkpoint`，但`next_action`仍指向原任务的唯一后续动作。不得把“回复用户状态”写成业务任务的最终下一动作。
- 工作仍可继续时，先用commentary给出中间状态，再在同一回合继续执行至少一个安全、明确的原子工作单元，或启动并登记已计划的长命令。不得仅因已经给出一次状态汇总就结束未完成任务。
- 若观察性问题到达时已有长命令运行，先读取新输出并说明状态，不为回复而中断命令；命令结束或产生可复核阶段结果后立即检查点。
- 只有任务已完成、用户明确暂停/取消、出现需要新授权的真实阻塞，或必须由用户作出会改变结果的选择时，才把控制权作为停点交回用户。普通失败候选、性能耗时、仍有F级实例或下一步需要继续搜索都不是停点。
- 用户消息确实改变目标、范围或约束时，按`revise`处理；不要把观察性提问误判为范围修订。
...
- 不把状态查询、中间汇报或原因解释当成暂停指令；活动任务回复后仍从原`next_action`继续。
