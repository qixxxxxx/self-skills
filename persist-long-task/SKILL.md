...
description: 通过项目内状态文件、原子检查点、恢复门禁和证据驱动的步骤转换，使长任务在上下文压缩、Goal 自动续跑、会话恢复、工具中断或 Agent 交接后从唯一下一动作继续，避免重做已完成工作。用于预计跨多轮或长时间、步骤较多、修改多个文件、运行长命令，或用户要求持续执行、Plan/Goal 配合、断点续做、压缩后恢复及 Agent 自行维护进度文件的任务。
---

# 长任务持久化协议

把磁盘状态当作执行真相源；聊天记忆、Plan 和 Goal 都只是派生视图。任何恢复场景都先读状态、核对现场，再继续唯一的 `next_action`。
...
2. 检查默认状态文件。若已存在：
   - 目标相同：恢复，不得重新初始化。
   - `schema_version` 为 `1` 或 `2`：先运行 `migrate` 建立 schema 3 恢复基线，再恢复。
   - 目标不同且旧任务未完成：停止并询问用户，不得覆盖。
   - 旧任务已完成：为新任务选择用户确认的独立状态目录，或在用户明确授权后处理旧状态。
3. 把工作拆成可独立验收的步骤。每步应能在一次合理工作单元内完成；长步骤继续拆分。
...
3. 对照 `current_step`、`last_checkpoint`、`changed_files` 和 `next_action`：
   - 证据已证明步骤完成：补记完成状态，不得重做。
   - 有部分产物：从缺失部分继续，不得从步骤开头覆盖重做。
   - 状态与现场矛盾：先记录检查点或阻塞，查明后再写业务文件。
...
5. 只有获得可复核证据后才运行 `complete`。不得只因代码已写完就标记完成：
...
6. 若用户调整目标、范围或约束，先用 `revise` 记录原因和新的下一动作，再同步 Goal/Plan。重大变化未获明确授权时先询问用户。
7. 若计划变化，使用 `add-step` 增补，或用 `skip` 同时记录理由和证据；`blocked` 步骤也可直接跳过并保留“因取消而解除阻塞”的记录。不要直接改 JSON，也不得越过更早的 pending 步骤。
8. 若受阻，使用 `block --reason ... --next-action ...`；恢复后用 `unblock`。阻塞状态也必须保留唯一下一动作。
...
- 不依赖不断增长的聊天或事件全文；恢复所需的当前步骤、证据、决定、验证和阻塞必须保留在 `state.json`，`journal.json` 只保存最新可恢复快照。
